import { Component, OnInit, AfterViewInit, ViewChild } from '@angular/core';
import { FormGroup, FormControl } from '@angular/forms';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ToastrService } from 'ngx-toastr';
import * as pdfMake from 'pdfmake/build/pdfmake';
import * as pdfFonts from 'pdfmake/build/vfs_fonts';

import { FormBuilder, Validators } from '@angular/forms';
import { MatDialog, MatDialogRef, MAT_DIALOG_DATA } from '@angular/material/dialog';

import { DialogReservaFichasComponent } from './dialog-reserva-fichas/dialog-reserva-fichas.component';
import { EmergenciasService } from '../../emergencias.service';


(<any>pdfMake).vfs = pdfFonts.pdfMake.vfs;


export interface Animal {
  name: string;
  sound: string;
}


@Component({
  selector: 'app-examenes-complementarios',
  templateUrl: './examenes-complementarios.component.html',
  styleUrls: ['./examenes-complementarios.component.scss']
})
export class ExamenesComplementariosComponent implements OnInit {

  /////////////////   VARIABLES  ///////////////////////
  ID_PRESTACION = 1388482;

  
  new_contenedor_data = true;
  list_laboratorio: FormGroup | any;
  imagenologia: FormGroup | any;
  data_respuesta: any;
  resp_grabar_formulario: any = [];
  Resp_listado_examenes: any = [];

  //////////servicios/////////
  sql: any;
  responde: any;



  //animalControl = new FormControl<Animal | null>(null, Validators.required);
  selectFormControl = new FormControl('', Validators.required);
  grupoSanguineo = [
    {id: 'O-', value: 'O-'},
    {id: 'O+', value: 'O+'},
    {id: 'A-', value: 'A-'},
    {id: 'A+', value: 'A+'},
    {id: 'B-', value: 'B-'},
    {id: 'B+', value: 'B+'},
    {id: 'AB-', value: 'AB-'},
    {id: 'AB+', value: 'AB+'},
  ];

  constructor(
    private fb: FormBuilder,
    private toastr: ToastrService,
    private EmergenciasService: EmergenciasService, 
    public dialog: MatDialog) {
    //this.dataSource = new MatTableDataSource();
  }

  ngOnInit(): void {
  }

  

  laboratorioForm = this.fb.group({

    l1: [''],
    l2: [''],
    l3: [''],
    l4: [''],
    l5: [''],
    l6: [''],
    l7: [''],
    l8: [''],
    l9: [''],
    l10: [''],
    l11: [''],
    l12: [''],
    l13: [''],
    l14: [''],
    l15: [''],
    l16: [''],
    l17: [''],
    l18: [''],
    l19: [''],
    l20: [''],
    l21: [''],
    l22: [''],
    l23: [''],
    l24: [''],
    l25: [''],
    l26: [''],
    l27: [''],
    l28: [''],
    l29: [''],
    l30: [''],
    l31: [''],

    ecografia: [''],
    rayosX: [''],

    tac: [''],
    endoscopia: [''],
    uno: [''],
    dos: [''],
    tres: [''],
    cuatro: [''],

    grupoSanguineo: [''],
    sangreTotal: [''],
    criopre: [''],
    paqueteGlobular: [''],
    anticuerposIrregul: [''],
    plasmaCongelado: [''],
    plasmaNormal: [''],
    globulosRojLavados: [''],
    aferesis: [''],
    concentradoPlaquetas: [''],
    activeotros: [''],
    otros: [''],

  }); 

  onSubmitLaboratorio() {
    const body = this.laboratorioForm.value;
    //body.cnsl_usuario_id = 1;
    //body.cnsl_hsp_id = 10;
    console.log(body);
  }



  openDialog() {
    const dialogRef = this.dialog.open(DialogReservaFichasComponent, {
      disableClose: true,
      width: '80%',
      //data: { campos: '},
    });
    dialogRef.afterClosed().subscribe(result => {
      console.log('The dialog was closed', result);
      /*this.data_respuesta = result;
      console.log('this.animal', this.data_respuesta);*/
    });
  }  

  grabar_formulario(){
    console.log('GRABANDO FORMULARIO');
    console.log('RAYOS X', this.laboratorioForm.value.rayosX);
    console.log('ECOGRAFIA', this.laboratorioForm.value.ecografia);
    if (this.laboratorioForm.value.rayosX == true || this.laboratorioForm.value.ecografia == true) {
      this.openDialog();
      console.log('DATOS PARA GUARDAR POR VERDAD', this.laboratorioForm.value);
      console.log('DATOS PARA GUARDAR POR VERDAD', JSON.stringify(this.laboratorioForm.value));
      this.sql = {
        consulta: 'select * from sp_insertar_examenes_complementarios('+ this.ID_PRESTACION +',$$'+ JSON.stringify(this.laboratorioForm.value) +'$$)'
      };
      this.EmergenciasService.dinamico(this.sql).subscribe(res => {
        this.responde = res as { message: string };
        if (this.responde.success.data[0].sp_dinamico != null) {
          this.resp_grabar_formulario = this.responde.success.data[0].sp_dinamico;
          console.log('RESPUESTA DE GUARDAR FORMULARIO', this.resp_grabar_formulario);
        } else {
          console.log('ERROR');
        }
      });
    } else {
      console.log('DATOS PARA GUARDAR POR FALSO', this.laboratorioForm.value);
      console.log('DATOS PARA GUARDAR POR VERDAD', JSON.stringify(this.laboratorioForm.value));
      this.sql = {
        consulta: 'select * from sp_insertar_examenes_complementarios('+ this.ID_PRESTACION +',$$'+ JSON.stringify(this.laboratorioForm.value) +'$$)'
      };
      this.EmergenciasService.dinamico(this.sql).subscribe(res => {
        this.responde = res as { message: string };
        if (this.responde.success.data[0].sp_dinamico != null) {
          this.resp_grabar_formulario = this.responde.success.data[0].sp_dinamico;
          console.log('RESPUESTA DE GUARDAR FORMULARIO', this.resp_grabar_formulario);
        } else {
          console.log('ERROR');
        }
      });
    }
  }

  imprimir_formulario(){
    console.log('IMPRIMIENDO');
    this.sql = {
      //consulta: 'select * from sp_lst_emergencias_examenes_complementarios(1388462)'
      consulta: 'select * from sp_lst_examenes_complementarios('+ this.ID_PRESTACION +')'
    };
    this.EmergenciasService.dinamico(this.sql).subscribe(res => {
      this.responde = res as { message: string };
      if (this.responde.success.data[0].sp_dinamico != null) {
        this.Resp_listado_examenes = this.responde.success.data[0].sp_dinamico;
        console.log('RESPUESTA DEL LISTADO', this.Resp_listado_examenes);
      } else {
        console.log('ERROR');
      }
    });




    const pdfDefinition: any = {
      content: [
        {
          text: '2022-09-15',
          style: 'fecha'
        },
        {
          columns: [
            {
              image: 'data:image/jpeg;base64,'
              ,width: 120,
              height: 120,
            },
              
            [
              { 
                text: '\n\n\n SOLICITUD DE EXAMENES DE LABORATORIO IMAGENOLOGIA /',
                style: 'header'
              },
              {
                text: '\n GABINETE Y SERVICIO DE SANGRE SEGURA',
                style: 'header'
              }
            ],
            {
              image:'data:image/png;base64,',
              width: 90,
              height: 90,
            }
          ]
        },
        {
          style: 'tableExample',
          color: '#444',
          table: {
            widths: ['*', '*', '*'],
            headerRows: 2,
            body: [
              [
                {text: 'RED:', style: 'tableHeader', alignment: 'left', colSpan: 2}, {},
                {text: 'MUNICIPIO:', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {text: 'ESTABLECIMIENTO:', style:'tableHeader', alignment: 'left', colSpan: 2}, {},
                {text: 'TIPO PACIENTE:', style:'tableHeader', alignment: 'left'}
              ],
              [
                {text: 'FECHA DE SOLICITUD:', style: 'tableHeader',alignment: 'left', colSpan: 2}, {},
                {text: 'N° DE REGISTRO:', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {text: 'NOMBRE:', style:'tableHeader', alignment: 'left'},
                {text: 'EDAD:', style:'tableHeader', alignment: 'left'},
                {text: 'SEXO:', style:'tableHeader', alignment: 'left'}
              ],
              [
                {text: 'DIAGNOSTICO CLINICO PRINCIPAL:', style:'tableHeader', alignment: 'left', colSpan: 3},
                {},
                {}
              ]
            ]
          }
        },
        {text: 'LABORATORIO - SIRVASE REALIZAR', style: 'subheader'},
        {
          style: 'tableExample',
          color: '#444',
          table: {
            //widths: ['*', '*', '*', '*', '*', '*'],
            widths: [25, 10, 200, 25, 10, 200],
            body: [
              [
                {text: 'PL8', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false],text: 'Baciloscopia seriada', style:'tableHeader', alignment: 'left'}, 
                {text: 'PL8', style: 'tableHeader', alignment: 'left'}, 
                {text: '', style: 'text', alignment: 'left'}, 
                {border: [false, false, false, false], text: 'inmunoglobulinas lgG, lgM, lgA', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {text: 'PL9', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Bilirrubinas totales y fracciones', style: 'tableHeader', alignment: 'left'},
                {text: 'PL9', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Moco fecal', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL18', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Coproparasitológico Simple', style: 'tableHeader', alignment: 'left'},
                {text: 'PL18', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Nitrogeno ureico sérico y úrea', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL19', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Coproparasitológico Seriado', style: 'tableHeader', alignment: 'left'},
                {text: 'PL19', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Proteina C Reactiva - PCR', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL21', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Creatinina en orina', style: 'tableHeader', alignment: 'left'},
                {text: 'PL21', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Proteinuria de 24 horas', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL22', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Creatinina sérica', style: 'tableHeader', alignment: 'left'},
                {text: 'PL22', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Prueba de Coombs directa/indirecta', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL15', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Cultivo p/germenes comunes y antibiograma', style: 'tableHeader', alignment: 'left'},
                {text: 'PL15', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Prueba rápida para sifilis', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL28', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Examen general de orina', style: 'tableHeader', alignment: 'left'},
                {text: 'PL28', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Tiempo de coagulación y tiempo de sangría', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL29', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Factor Reumatoide', style: 'tableHeader', alignment: 'left'},
                {text: 'PL29', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Tiempo de protrombina/TPP', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL32', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Fostatasa alcalina y acida', style: 'tableHeader', alignment: 'left'},
                {text: 'PL32', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Tinción PAP', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL34', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Frotis tincion Gram', style: 'tableHeader', alignment: 'left'},
                {text: 'PL34', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Transaminasas TGO-TGP', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL40', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Grupo sanguineo y factor Rh', style: 'tableHeader', alignment: 'left'},
                {text: 'PL40', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Test de embarazo en sangre/HCG', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL36', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Glicemia', style: 'tableHeader', alignment: 'left'},
                {text: 'PL36', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Reactantes de fase aguda(VES, Fibrinogeno y PCR)', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL39', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Gota gruesa y frotis sanguineo + tinción', style: 'tableHeader', alignment: 'left'},
                {text: 'PL39', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Reacción Widal', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL41', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Hemoglobina y hematocrito', style: 'tableHeader', alignment: 'left'},
                {text: 'PL41', style:'tableHeader', alignment: 'left'},
                {text: '', style:'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'RPR para Sifilis - VDRL', style:'tableHeader', alignment: 'left'} 
              ],
              [
                {text: 'PL42', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {border: [false, false, false, false], text: 'Hemograma completo', style: 'tableHeader', alignment: 'left'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'} 
              ]
            ]
          }
        },
        {
          style: 'tableExample',
          color: '#444',
          table: {
            widths: ['*', '*'],
            body: [
              [
                {border: [false, false, false, false], text: 'IMAGENOLOGÍA - SIRVASE REALIZAR:', style: 'subheader'},
                {border: [false, false, false, false], text: 'OTROS ESTUDIOS DE LABORATORIO O GABINETE', style: 'subheader'}
              ]
            ]
          }
        },
        {
          style: 'tableExample',
          color: '#444',
          table: {
            widths: [85, 200, 200],
            body: [
              [
                {border: [false, false, false, false], text: 'Ecografia', style: 'tableHeader', alignment: 'left'}, 
                {text: '', style: 'text', alignment: 'left'},
                {text: '1.', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {border: [false, false, false, false], text: 'Rayos X', style: 'tableHeader', alignment: 'left'}, 
                {text: '', style: 'text', alignment: 'left'},
                {text: '2.', style:'tableHeader', alignment: 'left'}
              ],
              [
                {border: [false, false, false, false], text: 'TAC', style: 'tableHeader', alignment: 'left'}, 
                {text: '', style: 'text', alignment: 'left'},
                {text: '3.', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {border: [false, false, false, false], text: 'Endoscopia', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'left'},
                {text: '4.', style:'tableHeader', alignment: 'left'}
              ]
            ]
          }
        },
        {text: 'SERVICIO DE BANCO DE SANGRE - SIRVASE OTORGAR:', style: 'subheader'},
        {text: 'GRUPO SANGUINEO y FACTOR Rh:', style: 'subheader'},
        {
          style: 'tableExample',
          color: '#444',
          table: {
            widths: [40, 130, 20, 40, 130, 20, 40],
            body: [
              [
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}, 
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {text: 'N°', style: 'tableHeader', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {text: 'N°', style: 'tableHeader', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}, 
                {border: [false, false, false, false], text: 'Sangre Total', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {border: [false, false, false, false], text: 'Crioprecipitados', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}, 
                {border: [false, false, false, false], text: 'Paquete Globular', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {border: [false, false, false, false], text: 'Anticuerpos Irregulares', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}, 
                {border: [false, false, false, false], text: 'Plasma Fresco Congelado', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {border: [false, false, false, false], text: 'Plasma Normal', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}, 
                {border: [false, false, false, false], text: 'Concentrado de Plaquetas', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {border: [false, false, false, false], text: 'Globulos Rojos', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}
              ],
              [
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}, 
                {border: [false, false, false, false], text: 'Aféresis', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'},
                {border: [false, false, false, false], text: 'Otros(Especificar)', style: 'tableHeader', alignment: 'left'},
                {text: '', style: 'text', alignment: 'center'},
                {border: [false, false, false, false], text: '', style: 'tableHeader', alignment: 'left'}
              ]
            ]
          }
        },
        {
          style: 'tableExample',
          color: '#444',
          table: {
            widths: ['*','*'],
            body: [
              [
                {text:  [
                  { 
                    text: '\n\n\n\n-----------------------------------------------',
                    style: 'tableHeader'
                  },
                  {
                    text: '\nFirma y Sello del Solicitante',
                    style: 'tableHeader'
                  }
                ]
                },
                {text:  [
                  { 
                    text: '\n\n\n\n-----------------------------------------------',
                    style: 'tableHeader'
                  },
                  {
                    text: '\nSello\nEstablecimiento',
                    style: 'tableHeader'
                  }
                ]
                }
               ]
            ]
          }
        }
      ],
      styles: {
        header: {
          fontSize: 9,
          alignment: 'center',
          bold: true
        },
        bigger: {
          fontSize: 15,
          italics: true
        },
        fecha: {
          fontSize: 8
        },
        body:{
          alignment: 'justify',
        },
        firma: {
          fontSize: 10,
          alignment: 'center',
          bold: true
        },
        text: {
          fontSize: 8,
          alignment: 'center',
        },
        tableHeader:{
          fontSize: 8,
          alignment: 'center',
          bold: true
        },
        subheader: {
          fontSize: 9,
          bold: true,
          margin: [0, 10, 0, 5]
        },
      
    
      },
      defaultStyle: {
        columnGap: 20,
        outerWidth: 600,
        width:500,
        innerWidth:400
      }
    };

    const pdf = pdfMake.createPdf(pdfDefinition);
    pdf.open();

  } 
  
}
