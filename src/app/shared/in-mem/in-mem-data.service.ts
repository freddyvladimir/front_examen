...
export class InMemDataService implements InMemoryDbService {
  ...
  get(reqInfo: RequestInfo) {
    ...
    /*if (is(reqInfo, 'me/menu')) {
      ...
      return ajax('assets/data/menu.json?_t=' + Date.now()).pipe(
        map((response: any) => {
          console.log("------<response>", JSON.stringify(response.response.menu));
          console.log("url",url);
          return { headers, url, status: STATUS.OK, body: { menu: [
            {
  ...
  post(reqInfo: RequestInfo) {
    ...
    if (is(reqInfo, 'auth/logout')) {
      console.log("Entra al If",reqInfo);
  ...
  private logout(reqInfo: RequestInfo) {
    ...
    const { headers, url } = reqInfo;
    const response = {};
    console.log("--------->>>>>>>",reqInfo.utils.createResponse$(() => response));
