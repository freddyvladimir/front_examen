...
export class TokenService implements OnDestroy {
  ...
  valid(): boolean {
    ...
    this.verifToken = sessionStorage.getItem(this.keyIdtoken);    
    if(this.verifToken){
      return true;
    }else{
      return false;
  ...
  storageData(data:any):any{
    this.store.set(this.keyId, data);
  }
  storageToken(data:any):any{
    this.store.set(this.keyIdtoken, data);
  }
